...
def probability(pheromone, dist_matrix, i, j, alpha=1.0, beta=2.0): 
    distance = dist_matrix[i][j]
    # If the distance is zero, assign a small epsilon value to prevent division by zero
    if distance == 0:
        heuristic = 1.0 / 1e-6  
    else:
...
def construct_tour(pheromone, dist_matrix, num_cities, alpha, beta):
...
def update_pheromones(pheromone, ants_tours, ants_tour_lengths, rho=0.5, Q=100):
    ...
    for tour, length in zip(ants_tours, ants_tour_lengths):
        deposit = Q / length
        for i in range(len(tour)):
            j = (i + 1) % len(tour)  # next city (wrap around to the start)
            pheromone[tour[i]][tour[j]] += deposit
            pheromone[tour[j]][tour[i]] += deposit  # if the distance matrix is symmetric
...
# Main ACO Function 
def ant_colony():
    # Parameters for probability influence
    alpha = 1.0  # influence of pheromone
    beta = 2.0   # influence of heuristic (1/distance)
    rho = 0.5    # evaporation rate
    Q = 100      # constant for pheromone deposit amount

    pheromone = initialise_pheromones(num_cities, initial_pheromone=1.0)
    ...
    for it in range(max_it):
        ...
        for ant in range(num_ants):
            tour = construct_tour(pheromone, dist_matrix, num_cities, alpha, beta)
        ...
        pheromone = update_pheromones(pheromone, ant_tours, ant_lengths, rho, Q)
