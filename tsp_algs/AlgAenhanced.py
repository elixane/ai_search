...
# Set a maximum time to ensure code does not run for longer than a minute (allow 10s buffer time)
max_time = 50 

# Define class representing a state in A* search
class State:
    def __init__(self, city, visited, g=0, h=0, parent=None):
        self.city = city
...
# Precompute the minimum edge cost from each city to speed up heuristic
min_edges = []
for i in range(num_cities):
    # Collect all distances from city i to every other city
    distances = []
    for j in range(num_cities):
        if i != j:
    ...
    min_i = min(distances)
    min_edges.append(min_i)

# Enhancement: compute an approximate MST cost over a set of unvisited cities using a simple Prim's algorithm
def mst_cost(unvisited):
    # If there are 0 or 1 cities, MST cost is zero
    if len(unvisited) <= 1:
        return 0

    mst_set = set()   # set of cities included in the MST
    remaining = set(unvisited)   # cities not yet included in MST

    # Start the MST with an arbitrary city
    current = remaining.pop()
    mst_set.add(current)
    cost = 0                      

    # Continue until all cities are added to the MST
    while remaining:
        min_edge = float('inf')   # smallest edge found in this iteration
        next_city = None    # next city to add to the MST

        # Find the minimum cost edge connecting MST to a remaining city
        for u in mst_set:
            for v in remaining:
        ...
        # Add the chosen city and edge cost to the MST
        cost += min_edge
        mst_set.add(next_city)
...
# Enhancement: updated heuristic using MST + return to start
def heuristic(city, visited):
    # Find unvisited cities
    unvisited = set(range(num_cities)) - visited

    # If all cities are visited, return cost to start
    if not unvisited:
        return dist_matrix[city][0]

    # Cost to enter the unvisited set
    min_to_unvisited = min(dist_matrix[city][u] for u in unvisited)

    # Cost to connect unvisited cities using MST
    mst = mst_cost(unvisited)

    # Cost to return to the start city
    min_back_to_start = min(dist_matrix[u][0] for u in unvisited)

    # Total estimated remaining cost
    return min_to_unvisited + mst + min_back_to_start

# Reconstruct the path from final state
def find_path(node):
    path = []
    while node:
        # Insert at front to reverse path
        path.insert(0, node.city)
        node = node.parent
    return path

# 2-Opt local optimisation to improve a greedy partial tour
def two_opt(path):
    improved = True
    while improved:
        improved = False
        for i in range(1, len(path) - 2):
            for j in range(i + 1, len(path) - 1):
                # Calculate the cost before and after swapping
                before = dist_matrix[path[i-1]][path[i]] + dist_matrix[path[j]][path[j+1]]
                after = dist_matrix[path[i-1]][path[j]] + dist_matrix[path[i]][path[j+1]]
                if after < before:
                    # If the swap improves, perform it
                    path[i:j+1] = reversed(path[i:j+1])
                    improved = True
    return path

# Completes a partial tour greedily (used when timeout occurs before full solution)
def complete_partial_tour(node):
    visited = set()
    path = []

    # Reconstruct current partial tour
    current = node
    while current:
    ...
    # Enhancement: apply 2-Opt optimisation to the path
    path = two_opt(path)

    # Recompute total tour cost based on optimised path
    new_total_cost = 0
    for i in range(len(path)):
        next_i = (i + 1) % len(path)
        new_total_cost += dist_matrix[path[i]][path[next_i]]

    return path, new_total_cost


# Main A* search function with MST heuristic and upper bound pruning
def a_star(start_city):
    start_time = time.time()

    unvisited_list = []  
    visited_list = set()

    start_visited = {start_city}
    start_node = State(city=start_city, visited=start_visited, g=0)
    start_node.h = heuristic(start_city, start_visited)
    start_node.f = start_node.g + start_node.h
    heapq.heappush(unvisited_list, start_node)

    # Track best partial node found (most cities visited) and count states explored
    best_partial = None
    best_visited_count = 0
    # Enhancement: initialise upper bound for pruning
    ...
    while unvisited_list:
        ...
        if time.time() - start_time > max_time:
            print("Timeout reached. Returning best partial solution.")
            break

        current = heapq.heappop(unvisited_list)
        states_explored += 1

        # Count how many cities visited
        if len(current.visited) == num_cities:
            path = find_path(current)
            total_cost = current.g + dist_matrix[current.city][start_city]
        ...
        # Update best partial
        if len(current.visited) > best_visited_count:
            best_partial = current
            best_visited_count = len(current.visited)

        # Use sorted tuple of visited cities as key
        visited_key = (current.city, tuple(sorted(current.visited)))
        if visited_key in visited_list:
            continue
        visited_list.add(visited_key)

        # Generate successors by visiting unvisited citiesn
        for neighbour in range(num_cities):
            if neighbour in current.visited:
                continue  # Skip cities already visited

            new_visited = set(current.visited)
            new_visited.add(neighbour)
            new_g = current.g + dist_matrix[current.city][neighbour]

            neighbour_node = State(city=neighbour, visited=new_visited, g=new_g, parent=current)
            neighbour_node.h = heuristic(neighbour, new_visited)
            neighbour_node.f = neighbour_node.g + neighbour_node.h

            # Upper-bound pruning: Skip bad nodes
            if neighbour_node.f >= best_full_cost:
                continue

            heapq.heappush(unvisited_list, neighbour_node)

    # Return best full tour found if available
    if best_full_path:
        print(f"Found complete tour after exploring {states_explored} states.")
        return best_full_path, best_full_cost

    # Otherwise complete the best partial tour
    if best_partial:
        print(f"Timeout fallback: completing partial tour with {best_visited_count} cities after {states_explored} states.")
        return complete_partial_tour(best_partial)
    
    # Output message if no path found
    print("No path found.")
    return None, float('inf')

# Run the A* search starting at city 0
tour, tour_length = a_star(0)
