...
class State:
    def __init__(self, city, visited, g=0, h=0, parent=None):
        ...
        self.h = h                            # Heuristic estimate to complete tour
        self.f = g + h                        # Total estimated cost
        self.parent = parent                  # Parent node (for path construction)

    def __lt__(self, other):
        return self.f < other.f              # Needed for priority queue (heapq)
...
# Heuristic function using nearest unvisited + sum of greedy estimates (connecting unvisited cities)
def heuristic(city, visited):
    # Find unvisited cities
    unvisited = set(range(num_cities)) - visited  

    # If all cities have been visited, find cost back to start 
    if not unvisited:
        return dist_matrix[city][0] 

    # Cost to reach the nearest unvisited city 
    min_to_unvisited = min(dist_matrix[city][u] for u in unvisited)  

    # Sum of min edges of each unvisitied city 
    min_edges_sum = sum(min_edges[u] for u in unvisited)

    # Combine both estimates             
    return min_to_unvisited + min_edges_sum

# Select the path from goal node to start using parent pointers
def find_path(node):
    path = []
    while node:
...
# Complete a partial tour greedily (used when timeout occurs before full solution)
...
def a_star(start_city):
    unvisited_list = []    # priority queue (min-heap) of states to explore
    visited_list = set()     # set of visited states to avoid re-processing
    ...
    while unvisited_list:
        ...
        if len(current.visited) == num_cities:
            ...
            print(f"Completed optimal tour after exploring {states_explored} states.")
            return path, total_cost
        ...
        # Generate successors by visiting unvisited cities
    ...
    # If timeout was reached, complete and return best partial tour found
    if best_partial:
        print(f"Timeout fallback: completing partial tour with {best_visited_count} cities after exploring {states_explored} states.")
