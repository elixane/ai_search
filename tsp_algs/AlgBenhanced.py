...
# Parameters for the AC algorithm
max_it = 500      
num_ants = 10      
L = 10    # nearest neighbour candidate list size 

# Local‐update constants 
xi = 0.1    # local evaporation rate
tau0 = 1    # initial pheromone on every edge

# Dynamic-schedule endpoints
alpha_start = 0.5
alpha_end = 2
rho_start = 0.8
rho_end = 0.2
# Keep heuristic weight fixed
beta = 2.0   

 # Enhancement: For each city, return a list of up to L nearest neighbours
def choose_nearest(dist_matrix, num_cities, L): 
    nearest_n = []
    for i in range(num_cities):
        # Pair (distance, city) for all other cities
        dists = []
        for j in range(num_cities):
            if j != i:
                dists.append((dist_matrix[i][j], j))
        dists.sort(key=lambda x: x[0])
        
        # Take the first L cities
        neighbours = []
        for dist, city in dists[:L]:
            neighbours.append(city)
        
        nearest_n.append(neighbours)
    
    return nearest_n

# Create and return an initial pheromone matrix for the given number of cities 
def initialise_pheromones(num_cities, initial_pheromone=1.0):
    pheromone = []
    for i in range(num_cities):
        row = []
        for j in range(num_cities):
...
# Calculate the move probability from city i to city j 
def probability(pheromone, dist_matrix, i, j, alpha, beta):
    distance = dist_matrix[i][j]
    # If the distance is zero, we can either assign a small epsilon value to prevent division by zero
    if distance == 0:
        heuristic = 1.0 / 1e-6  
    else:
        heuristic = 1.0 / distance
    return (pheromone[i][j] ** alpha) * ((heuristic) ** beta)


# Construct a tour for one ant based on current pheromone and heuristic values
def construct_tour(pheromone, dist_matrix, num_cities, nearest_n, alpha, beta):
    ...
    # Restricted choices of neighbours to visit (enhancement: only candidate-list neighbours)
    while unvisited:
        # Build the restricted-neighborhood list
        neighbour = []
        for c in nearest_n[current]:
            if c in unvisited:
                neighbour.append(c)

        # Choose between restricted list and full unvisited set
        if neighbour:
            choices = neighbour
        else:
        ...
        probabilities = []
        total = 0
        for next_city in choices:
            p = probability(pheromone, dist_matrix, current, next_city, alpha, beta)
        ...
        cumulative = 0
        for next_city, p in probabilities:
            cumulative += p
            if cumulative >= r:
                chosen = next_city
                break
        
        # Enhancement: local pheromone update 
        pheromone[current][chosen] = (1 - xi)*pheromone[current][chosen] + xi*tau0
        pheromone[chosen][current] = pheromone[current][chosen]
...
def update_pheromones(pheromone, ants_tours, ants_lens, rho, Q=100):
    n = len(pheromone)
    # Evaporation: reduce all pheromone values
    for i in range(n):
        for j in range(n):
            pheromone[i][j] *= (1 - rho)

    # Deposition: add pheromones according to tour quality
    for tour, length in zip(ants_tours, ants_lens):
        deposit = Q / length
...
# Main ACO with Local Update + Dynamic Parameter Scheduling
def ant_colony():
    pheromone = initialise_pheromones(num_cities, tau0)
    nearest_n = choose_nearest(dist_matrix, num_cities, L)
    
    best_tour, best_length = None, float('inf')
    
    for it in range(max_it):
         # Linear interpolation for alpha, rho (enhancement: dynamic parameter scheduling)
        frac = it / (max_it - 1)
        alpha = alpha_start + (alpha_end - alpha_start) * frac
        rho = rho_start + (rho_end - rho_start) * frac
        ...
        for ant in range(num_ants):
            tour = construct_tour(pheromone, dist_matrix, num_cities, nearest_n, alpha, beta)
        ...
        # Update pheromone levels based on all tours
        pheromone = update_pheromones(pheromone, ant_tours, ant_lengths, rho)

    return best_tour, best_length

# Execute the ACO algorithm
tour, tour_length = ant_colony()
